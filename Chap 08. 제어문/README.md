...
블록문은 단독으로도 사용이 가능하다.  
또한 블록문으 자체 종결성을 갖기 때문에 끝에 세미콜론을 붙이지 않는다.
...
반복문(loop statement)은 조건식의 평가 결과가 참인 경우 코드 블록을 실행한다. 이후 조건식을 다시 평가하여 참인경우 코드블록을 재실행 한다. => "반복"  
...
조건식의 평가 결과가 불리언 값이 아니면 불리언 값으로 강제 변환하여 논리적 참, 거짓을 구별한다. (truthy, falsy)
...
따라서 조건식부터 평가하는 while문과 달리, 반드시 1번은 코드블록을 실행하게 된다.
